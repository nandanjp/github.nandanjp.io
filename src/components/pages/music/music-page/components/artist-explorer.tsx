...
function computeLayout(n: number): { viewBox: string; cy: number } {
    ...
    const yOffsets = getAngles(n).map(
...
function ArtistGraph({ artistName, albums }: ArtistGraphProps) {
    ...
    const displayName =
        artistName.length > 20 ? artistName.slice(0, 19) + '…' : artistName
    ...
    if (n === 1) {
        ...
        return (
            ...
            >
                ...
                <motion.g
                    style={{
                        transformOrigin: `${albumCX}px ${albumCY}px`,
                        cursor: 'pointer'
                    }}
...
export function ArtistExplorer({ tracks }: { tracks: Track[] }) {
    ...
    for (const track of tracks) {
        for (const artist of track.artists) {
            if (!artistAlbumMap.has(artist))
                artistAlbumMap.set(artist, new Map())
            const albumMap = artistAlbumMap.get(artist)!
            if (!albumMap.has(track.album_art_url))
                albumMap.set(track.album_art_url, track)
        }
    }
    const artists = [...artistAlbumMap.keys()].sort(
        (a, b) => artistAlbumMap.get(b)!.size - artistAlbumMap.get(a)!.size
    )
    const selectedAlbums = selected
        ? [...artistAlbumMap.get(selected)!.values()]
        : []
