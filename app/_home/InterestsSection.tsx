...
function EntryOutput({ entry }: { entry: Entry }) {
        ...
        return (
            <div className="flex flex-col gap-0.5 font-mono text-xs">
                {SITE.home.interests.languages.map(lang => (
                    <span key={lang.label}>
                        <span className="text-muted-foreground/60 inline-block w-16">{lang.label}</span>
                        <span className="text-foreground/80">{lang.level}</span>
                        {'note' in lang && (
                            <span className="text-muted-foreground ml-2 text-xs">{lang.note}</span>
                        )}
                    </span>
                ))}
            </div>
    ...
    return (
        <p className="text-muted-foreground/70 font-mono text-xs">{entry.text}</p>
    )
}
